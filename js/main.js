	...
	!function (t, i, e, s) {
		...
		function h(s) {
			return this.each(function () {
				var h = t(this),
				r = "object" == typeof s && s;
				this == i || this == e || h.is("body") ? o.configure(r) : h.data("px.parallax") || (r = t.extend({}, h.data(), r), h.data("px.parallax", new o(this, r))),
				"string" == typeof s && o[s]()
			})
		}
		!function () {
...
(function ($) {
	...
	window.last_scrolled = window.scrolled;
	var page_down_pos_top = $(".down-arrow").length ? $(".down-arrow").offset().top + $(".down-arrow").height() : '';
	...
	$.fn.parallaxScroll = function () {
		...
		if (last_scrolled > window.scrolled) {
			direction = -1;
		} else {
			direction = 1;
		}
		last_scrolled = window.scrolled;

		/* page down parallax treatment */
		var page_down = $(".down-arrow");

		if (window.scrolled > (page_down_pos_top - window.window_height)) {
			page_down.addClass("down-arrow-fixed pos-fixed");
			$('.openhouse').addClass('down-arrow-padding');
		} else {
			page_down.removeClass("down-arrow-fixed pos-fixed");
			$('.openhouse').removeClass('down-arrow-padding');
		}
		...
		if (window.scrolled + window.window_height > $('#ubc7-footer').offset().top) {
			page_down.addClass("hidden");
		} else {
			page_down.removeClass("hidden");
		}
	};

	/* page down jump link */
	$(".down-arrow i").on("click", function (e) {
		if ($("html").scrollTop() >= 0) {
			$("body").animate({
				scrollTop : (window.scrolled + window.window_height)
			}, 1000);
		} else {
			$("html").animate({
				scrollTop : (window.scrolled + window.window_height)
